...
owl_tcp_active_true_test() ->
	...
	ok = owl_tcp:send_stanza( Owl, SaslAuthRequest1 ),
	{ok, ?xmpp_stanza( Owl, MaybeSaslAuthResponse1 )} = owl_tcp:receive_xmpp_event( Owl ),
	...
	ok = owl_tcp:send_stanza( Owl, SaslAuthRequest2 ),
	{ok, ?xmpp_stanza( Owl, MaybeSaslAuthResponse2 )} = owl_tcp:receive_xmpp_event( Owl ),
...
owl_tcp_active_once_test() ->
	{ok, Owl} = owl_tcp:start_link( tcp_connect, "localhost", 5222, [] ),
	ok = owl_tcp:send_stream_open( Owl, [ {<<"to">>, <<"admin.localhost">>} ] ),

	ok = owl_tcp:set_active( Owl, once ),
	{ok, ?xmpp_stream_open( Owl, _StreamAttrs1 )} = owl_tcp:receive_xmpp_event( Owl ),
	...
	ok = owl_tcp:send_stream_open( Owl, [ {<<"to">>, <<"admin.localhost">>} ] ),
	ok = owl_tcp:set_active( Owl, once ),
	{ok, ?xmpp_stream_open( Owl, _StreamAttrs2 )} = owl_tcp:receive_xmpp_event( Owl ),

	ok = owl_tcp:set_active( Owl, true ),
	PresenceRequest = owl_stanza_presence:stanza_new( available ),
	ok = owl_tcp:send_stanza( Owl, PresenceRequest ),
	{ok, ?xmpp_stanza( Owl, _PresenceEcho )} = owl_tcp:receive_xmpp_event( Owl ),
