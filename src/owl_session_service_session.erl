-module (owl_session_service_session).
...
handle_info_stanza_session_response( ResponseIQ, S0 ) ->
	case owl_stanza_iq:type( ResponseIQ ) of
		result ->
			S1 = S0 #s{ session = true },
			ok = owl_session_service:notify_facility_available( session, ok, true ),
			{noreply, S1};
		error ->
			ok = owl_session_service:notify_facility_available( session, error, ResponseIQ ),
...
do_send_session_request( IQID, S0 = #s{ session_srv = SessionSrv } ) ->
	...
	RequestIQ = owl_stanza_iq:request_new( set, IQID, SessionElement ),
	ok = owl_session:send_stanza( SessionSrv, RequestIQ ),
	{ok, S0}.
